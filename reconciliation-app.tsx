...
function ReportPage({ bank, cashier, results, onBack }: { bank: BankRow[]; cashier: CashierRow[]; results: MatchResult[] | null; onBack: () => void }) {
  const bankTotal = bank.reduce((sum, row) => sum + row.rawAmount, 0);
  const cashierTotal = cashier.reduce((sum, row) => sum + row.matchAmount, 0);
  const difference = bankTotal - cashierTotal;
  const matched = results?.filter(row => row.type === "saved").length ?? 0;
  const categories = [
    { label: "المطابقات المؤكدة", value: matched, amount: results?.filter(row => row.type === "saved").reduce((sum, row: any) => sum + (row.bank?.rawAmount || 0), 0) ?? 0, tone: "gold" },
    { label: "مطابقات تحتاج مراجعة", value: results?.filter(row => row.type === "pending").length ?? 0, amount: results?.filter(row => row.type === "pending").reduce((sum, row: any) => sum + (row.bank?.rawAmount || 0), 0) ?? 0, tone: "blue" },
    { label: "فواتير بلا حوالات", value: results?.filter(row => row.type === "unmatchedCashier").length ?? 0, amount: results?.filter(row => row.type === "unmatchedCashier").reduce((sum, row: any) => sum + (row.cashier?.matchAmount || 0), 0) ?? 0, tone: "red" },
    { label: "حوالات بلا فواتير", value: results?.filter(row => row.type === "unmatchedBank").length ?? 0, amount: results?.filter(row => row.type === "unmatchedBank").reduce((sum, row: any) => sum + (row.bank?.rawAmount || 0), 0) ?? 0, tone: "violet" },
    { label: "فيزا", value: results?.filter(row => row.type === "visa").length ?? 0, amount: results?.filter(row => row.type === "visa").reduce((sum, row: any) => sum + (row.cashier?.matchAmount || 0), 0) ?? 0, tone: "teal" },
    { label: "محفظة محمود", value: results?.filter(row => row.type === "mahmoudWallet").length ?? 0, amount: results?.filter(row => row.type === "mahmoudWallet").reduce((sum, row: any) => sum + (row.cashier?.matchAmount || 0), 0) ?? 0, tone: "amber" },
    { label: "جوال بي", value: results?.filter(row => row.type === "jawwalPay").length ?? 0, amount: results?.filter(row => row.type === "jawwalPay").reduce((sum, row: any) => sum + (row.cashier?.matchAmount || 0), 0) ?? 0, tone: "cyan" },
  ];
  const maxAmount = Math.max(...categories.map(category => category.amount), 1);
  return (
    <div dir="rtl" className="min-h-[100dvh] bg-background p-4 sm:p-8">
            ...
            <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
              <section className="rounded-2xl border bg-card p-5 sm:p-6">
                ...
                <div className="space-y-5">
                  {categories.map(category => <div key={category.label}><div className="mb-2 flex items-center justify-between gap-3 text-sm"><span className="font-semibold">{category.label}</span><span className="font-mono text-xs text-muted-foreground">{fmtNum(category.amount)} ₪ · {category.value}</span></div><div className="h-3 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full report-bar-${category.tone}`} style={{ width: `${Math.max(category.value ? 4 : 0, category.amount / maxAmount * 100)}%` }} /></div></div>)}
                </div>
              </section>
...
export default function App({ initialPage = "recon2" }: { initialPage?: PageId } = {}) {
  ...
  const handleSaveMatch = (
    ...
    let patchedExistingPending = false;
    setResults(prev => prev
      ? prev.map(result => {
          if (result.type === "pending" && result.cashier.id === cashierRow.id && result.bank.id === bankRow.id) {
            patchedExistingPending = true;
            return { type: "saved", cashier: cashierRow, bank: bankRow, savedMatch: newSaved };
          }
          return result;
        })
      : prev);
    if (!patchedExistingPending) {
      // This save didn't come from an already-visible "pending" suggestion —
      // e.g. it was picked in مساعد المطابقة for a previously unmatched
      // invoice. The cheap patch above can't move it out of the unmatched
      // lists, so let the normal reconciliation pass rebuild everything
      // (same as finishReplacement does for the same reason).
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
               {([
                 ...
                 { id: "manual", label: "يدوي", count: stats.manual, color: "text-amber-700" },
                 ...
                 { id: "held", label: "معلّقة", count: stats.held, color: "text-purple-700" },
                 { id: "uCashier", label: "فواتير بلا حوالات مطابقة", count: stats.uCashier, color: "text-red-700" },
                 { id: "uBank", label: "حوالات بلا فواتير مسجلة", count: stats.uBank, color: "text-orange-700" },
              ] as const).map(t => (
